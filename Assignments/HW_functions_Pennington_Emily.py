...
x = 2021
for m in range(4):
  print(monthofyear[(m+8)%12], x)
for index, val in enumerate(monthofyear):
  print(val, str(x+1))
for index, val in enumerate(monthofyear):
  print(val, str(x+2)) 
for m in range(5):
  print((monthofyear[(m)%12]),str(x+3))
...
def find_grade(total):
  ...
  :return: str
  '''
  grade = "A" if total >= 93 else "A-" if total >= 90 else "B+" if total >= 87 else "B"if total >= 83 else "B-"if total >= 80 else "C+" if total >= 77 else "C" if total >= 73 else "C-" if total >= 70 else "D" if total >= 63 else "D-" if total >= 60 else "F"
  return grade 
...
# which convert a letter grade to a grade point. A is 4.0, A- is 3.7, B+ = 3.3, B = 3.0, B- = 2.7, C+ = 2.3, C = 2.0, C- = 1.7, D+ = 1.3, D = 1.0, D- = 0.7, F = 0 etc
grade = 'C-'
